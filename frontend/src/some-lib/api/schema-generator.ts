...
export async function generateSchemaTypes(): Promise<SchemaTypes> {
  try {
    const schemas = await defaultApiClient.request<
      { name: string; tables: { [key: string]: TableMetadata } }[]
    >("/dt/schemas");
    ...
    for (const schema of schemas) {
      ...
      for (const [tableName, tableMetadata] of Object.entries(schema.tables)) {
        ...
        for (const column of tableMetadata.columns) {
          schemaTypes[schema.name][tableName][column.name] = mapPgTypeToTs(
            column.type,
          );
        }
      }
