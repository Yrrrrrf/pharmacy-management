 ...
 */
export function generateCrudOperations<T>(
  schema: string,
  table: string,
): CrudOperations<T> {
  ...
  return {
    getAll: (params = {}) =>
      defaultApiClient.request<T[]>(basePath, { params }),
    getOne: (id) => defaultApiClient.request<T>(`${basePath}/${id}`),
    create: (data) =>
      defaultApiClient.request<T>(basePath, {
        method: "POST",
        body: JSON.stringify(data),
      }),
    update: (id, data) =>
      defaultApiClient.request<T>(`${basePath}/${id}`, {
        method: "PUT",
        body: JSON.stringify(data),
      }),
    delete: (id) =>
      defaultApiClient.request(`${basePath}/${id}`, { method: "DELETE" }),
  };
}
